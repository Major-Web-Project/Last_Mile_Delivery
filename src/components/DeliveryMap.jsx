...
const DeliveryMap = ({ 
  ...
  const mockLocations = [
    ...
    { lat: currentLocation?.lat || 40.6782, lng: currentLocation?.lng || -73.9442, label: "Current Location" },
  ];
  ...
  return (
    <div className="relative w-full h-full">
      <div ref={mapRef} className="w-full h-full bg-slate-100 rounded-lg flex items-center justify-center">
        {/* Mock Map Interface */}
        <div className={`w-full h-full relative bg-gradient-to-br from-blue-50 to-green-50 rounded-lg overflow-hidden ${sosActive ? 'border-4 border-red-500 animate-pulse' : ''}`}>
          {/* SOS Alert Overlay */}
          {(sosActive || sosAlerts.length > 0) && (
            <div className="absolute top-0 left-0 right-0 bg-red-600 text-white p-2 z-10">
              <div className="text-center font-bold animate-pulse">
                🚨 EMERGENCY ALERT - Agent Location Being Tracked
              </div>
            </div>
          )}
              ...
              >
                <div className={`w-6 h-6 rounded-full border-2 border-white shadow-lg flex items-center justify-center text-xs font-bold text-white ${
                  index === mockLocations.length - 1 
                    ? (sosActive ? 'bg-red-600 animate-pulse' : 'bg-blue-600') 
                    : 'bg-red-500'
                }`}>
                  {index === mockLocations.length - 1 ? '📍' : '🚚'}
                </div>
          ...
          {showRoute && (
            <svg className="absolute inset-0 w-full h-full pointer-events-none">
              <path
                ...
                stroke={sosActive ? "#DC2626" : "#3B82F6"}
                strokeWidth="3"
                fill="none"
                strokeDasharray="5,5"
          ...
          <div className="absolute top-4 left-4 bg-white rounded-lg shadow-sm p-3 space-y-2">
            ...
            <div className="flex items-center space-x-2 text-xs">
              <div className={`w-3 h-3 rounded-full ${sosActive ? 'bg-red-600' : 'bg-blue-600'}`}></div>
          ...
          {customerView && (
            <div className="absolute bottom-4 left-4 bg-white rounded-lg shadow-sm p-3">
              {sosAlerts.length > 0 ? (
                <div>
                  <div className="text-sm font-semibold text-red-600">🚨 Emergency Alert</div>
                  <div className="text-xs text-slate-600">Your delivery agent has sent an SOS</div>
                </div>
              ) : (
                <div>
          ...
          {dashboardView && (
            <div className="absolute top-4 right-4 bg-white rounded-lg shadow-sm p-3">
              ...
              <div className="text-xs text-slate-600">{deliveries?.length || 0} active deliveries</div>
              <div className="text-xs text-slate-600">{agents?.length || 0} agents online</div>
              {sosAlerts.length > 0 && (
                <div className="text-xs text-red-600 font-semibold">
                  🚨 {sosAlerts.length} Emergency Alert(s)
                </div>
              )}
            </div>
          )}
